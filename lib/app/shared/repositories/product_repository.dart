...
class ProductRepository implements IProductRepository {
  ...
  Stream<List<ProductModel>> getAll() async*{
    var productsStream = firestore.collection("product").orderBy("name").snapshots();
    ...
    await for (var productSnapshot in productsStream) {
      for (var productDoc in productSnapshot.docs) {
        ...
        DocumentSnapshot categorySnapshot = await firestore.collection("category").doc(categoryReference.id).get();
        product =  ProductModel.fromDocument(productDoc, CategoryModel.fromDocument(categorySnapshot));
  ...
  Future save(ProductModel productModel) async {
    // productModel.reference = await firestore.collection('category').add(
